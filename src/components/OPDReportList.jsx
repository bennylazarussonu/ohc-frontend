import { FaFileExcel, FaMagnifyingGlass, FaPenToSquare } from "react-icons/fa6";
...
function OPDReportList({ onEdit, refreshKey }) {
  ...
  const downloadExcel = () => {
    const data = filteredRecords.map(opd => ({
      ...
      "Handled By": opd?.case_dealt_by?.userId
    }));
    ...
    XLSX.writeFile(
      workbook,
      `OPD_Reports_${new Date().toISOString().split("T")[0]}.xlsx`
    );
  };
  ...
  return (
    <div className="bg-gray-800 rounded w-full text-xs">
      <div className="flex justify-between">
        <h2 className="text-sm font-bold">OPD Reports</h2>
        <button
      ...
      <table className="w-full text-xs">
        <thead className="border bg-gray-900">
          <tr className="border">
            ...
            <th className="border p-2 w-[20%]">Worker</th>
            <th className="border p-2">Designation</th>
            <th className="border p-1">Aadhar</th>
            <th className="border p-2">Date</th>
            <th className="border p-2 w-[15%]">Complaint</th>
